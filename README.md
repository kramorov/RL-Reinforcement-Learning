...
В качестве примера, вернемся к нашей сети перцептрона, только в выходном слое будет не один нейрон, а несколько:
model = Sequential()
model.add(Dense(10, activation='relu', input_shape=(n_features,)))
model.add(Dense(8, activation='relu', ))
...
В задачах классификации, обычно выбирается наибольшее значение из этого вектора, чтобы определить класс. Предположим, мы имеем на выходе вектор [0.2 0.3 0.4 0.1]. Наибольшее значение имеется в 3 по счету (или в индексе "2" массива), что позволяет нам сказать, что входные данные относятся к классу 3 ("2"). Строго говоря, 
...
1. "В строгом соответствии с политикой" (On-Policy Value Function), которая рассчитывает ожидаемый результат, если агент будет выполнять действия в строгом соответствии с политикой.
2. "Действие, затем в строгом соответствии с политикой" (On-Policy Action-Value Function). Рассчитывает ожидаемый результат, если агент сначала выполнит произвольное действие - возможно, не совпадающее с политикой, затем будет выполнять действия в строгом соответствии с политикой.
3. "В соответствии с политикой оптимальной ценности" (Optimal Value Function), ), которая рассчитывает ожидаемый результат, если агент будет выполнять действия в строгом соответствии с ОПТИМАЛЬНОЙ политикой.
4. "Действие, затем в соответствии с политикой оптимальной ценности" (Optimal Action-Value Function). Рассчитывает ожидаемый результат, если агент сначала выполнит произвольное действие - возможно, не совпадающее с политикой, затем будет выполнять действия в строгом соответствии с ОПТИМАЛЬНОЙ политикой.
Все варианты функции ценности ведут расчет от какого-то момента s.
...
1. В процессе обучения агент вырабатывает политику (стохастическую или детерминистскую);
